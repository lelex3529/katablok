...
interface EditBlockClientProps {
  blockId: string;
  initialBlock?: Block;
}

export default function EditBlockClient({
  blockId,
  initialBlock,
}: EditBlockClientProps) {
  const router = useRouter();
  const { block, loading, error, updateBlock, refreshBlock } =
    useBlock(blockId);
  ...
  // We can optionally use initialBlock from server for faster initial render
  const displayBlock = block || initialBlock;
      ...
      <div className='flex justify-between items-center'>
        ...
        {displayBlock && (
          <div className='hidden md:flex items-center px-4 py-2 bg-white rounded-xl shadow-sm text-sm text-gray-600 border border-gray-100'>
            ...
            Editing: {displayBlock.title}
          </div>
        )}
      </div>

      {loading && !displayBlock ? (
